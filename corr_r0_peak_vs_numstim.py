...
Data_sel = Data[:,:,stimuli]; Data_sel=Data_sel[s,:,:]
...
for i_n in range(n):
    ...
    for i_sub in range(nsubsamplings):
        ...
        r0 = Yrec_renorm[:,0]
        r0 = r0/norm(r0)
        tpeak = argmax(norm(Y1_renorm,axis=0))
...
for i_n in range(n):
    ...
    for i_sub in range(nsubsamplings):
        ...
        r0 = r0/norm(r0)
        Y1 = R[2][:,:time_steps]
        tpeak = argmax(norm(Y1,axis=0))
        rpeak = X_PC_P[:,tpeak]
        rpeak = rpeak/norm(rpeak)

        ic_peak_corr_RecurrentModel[i_sub,i_n] = abs(dot(r0,rpeak))
    print(mean(ic_peak_corr_RecurrentModel[:,i_n]))


figure(figsize=(2.2,2))

fill_between([nstim[0]-.5,nstim[-1]+.5], (mData-sData)*ones(2), (mData+sData)*ones(2),color='#e8e8e8')
plot([nstim[0]-.5,nstim[-1]+.5], (mData)*ones(2), lw=1.3, color='#819299', label='Data')
plot([1,16], 1/e*ones(2),'--', lw=1.2, color='grey')
